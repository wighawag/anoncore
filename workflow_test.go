...
// The CI workflow must honestly document the integration gap: anoncore's
// integration suite is behind `-tags integration`, needs root on a capable host,
// and is deliberately NOT run in GitHub CI.
func TestCIDocumentsIntegrationGap(t *testing.T) {
	wf := strings.ToLower(ciWorkflow(t))
	if !strings.Contains(wf, "integration") {
		t.Error("ci.yml must document the integration gap")
	}
	...
	if !strings.Contains(wf, "root") {
		t.Error("ci.yml must state the integration suite needs root")
	}
}
